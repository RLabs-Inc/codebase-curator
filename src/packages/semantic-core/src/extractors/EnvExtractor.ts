...
export class EnvExtractor implements LanguageExtractor {
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmedLine.startsWith('#')) {
        ...
        if (comment.length > 2) {
          ...
          if (
            /^[A-Z\s]+$/.test(comment) ||
            comment.includes('CONFIG') ||
            comment.includes('SETTINGS')
          ) {
      ...
      if (envMatch) {
        ...
        const relatedTerms = [
          category,
          ...this.extractRelatedTerms(key, cleanedValue),
        ]
        ...
        if (
          ...
          this.isMeaningfulValue(cleanedValue)
        ) {
          definitions.push({
            term: cleanedValue,
            type: 'string',
  ...
  private categorizeEnvVar(key: string, value: string): string {
    // Database
    if (
      /^(DB|DATABASE|POSTGRES|MYSQL|MONGO|REDIS)_/.test(key) ||
      key.includes('_DB_') ||
      key === 'DATABASE_URL'
    ) {
      return 'database'
    ...
    // API/URLs
    if (
      /^(API|ENDPOINT|URL|URI|HOST)_/.test(key) ||
      key.includes('_URL') ||
      key.includes('_API') ||
      value.includes('http://') ||
    ...
    // Authentication
    if (
      /^(AUTH|TOKEN|KEY|SECRET|PASSWORD|JWT|OAUTH)_/.test(key) ||
      key.includes('_KEY') ||
    ...
    // Feature flags
    if (
      /^(ENABLE|DISABLE|USE|FEATURE|FLAG)_/.test(key) ||
      key.includes('_ENABLED') ||
      key.includes('_DISABLED')
    ) {
      return 'feature'
    }
  ...
  private extractReferences(
    ...
    if (key.includes('PORT') && /^\d+$/.test(value)) {
      references.push({
        ...
        referenceType: 'config' as CrossReference['referenceType'],
    ...
    if (envVarMatch) {
      references.push({
        ...
        referenceType: 'variable' as CrossReference['referenceType'],
        fromLocation: { file: filePath, line, column: 0 },
        context: `${key}=${value}`,
      })
    }
    ...
    if (value.includes('http://') || value.includes('https://')) {
      try {
        ...
        references.push({
          ...
          referenceType: 'endpoint' as CrossReference['referenceType'],
          fromLocation: { file: filePath, line, column: 0 },
          context: `${key}=${value}`,
        })
      } catch {
