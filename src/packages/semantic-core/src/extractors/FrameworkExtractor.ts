...
export class FrameworkExtractor implements LanguageExtractor {
  ...
  private extractSvelteSections(
    ...
    const scriptRegex =
      /<script(?:\s+lang="([^"]+)")?(?:\s+context="module")?>([\s\S]*?)<\/script>/g
    let match
  ...
  private extractVueSections(
    ...
    const scriptRegex =
      /<script(?:\s+setup)?(?:\s+lang="([^"]+)")?>([\s\S]*?)<\/script>/g
    let match
    while ((match = scriptRegex.exec(content))) {
      const startLine = content.substring(0, match.index).split('\n').length
      sections.push({
    ...
    // Extract <style> sections
    const styleRegex =
      /<style(?:\s+scoped)?(?:\s+lang="([^"]+)")?>([\s\S]*?)<\/style>/g
    while ((match = styleRegex.exec(content))) {
      const startLine = content.substring(0, match.index).split('\n').length
      sections.push({
        type: 'style',
  ...
  private extractScriptSection(
    ...
    const fileType = this.getFileType(filePath)
    const frameworkResults = this.extractFrameworkFeatures(
      content,
      filePath,
      lineOffset,
      fileType
  ...
  private extractFrameworkFeatures(
    ...
    switch (fileType) {
      case 'svelte':
        ...
        while ((match = runeRegex.exec(content))) {
          ...
          definitions.push({
            ...
            surroundingLines: lines.slice(
              Math.max(0, lineNum - 2),
              lineNum + 2
            ),
        ...
        const lifecycleRegex =
          /\b(onMount|onDestroy|beforeUpdate|afterUpdate|tick)\s*\(/g
        while ((match = lifecycleRegex.exec(content))) {
          ...
          references.push({
            ...
            referenceType:
              'svelte-lifecycle' as CrossReference['referenceType'],
            fromLocation: {
              file: filePath,
              line: lineOffset + lineNum - 1,
              column: match.index - content.lastIndexOf('\n', match.index) - 1,
        ...
        // Svelte context API
        const contextRegex =
          /\b(setContext|getContext|hasContext|getAllContexts)\s*\(/g
        while ((match = contextRegex.exec(content))) {
          const lineNum = content.substring(0, match.index).split('\n').length
          const contextFn = match[1]
          references.push({
            targetTerm: contextFn,
            referenceType: 'svelte-context' as CrossReference['referenceType'],
            fromLocation: {
              file: filePath,
              line: lineOffset + lineNum - 1,
              column: match.index - content.lastIndexOf('\n', match.index) - 1,
        ...
        while ((match = storeRegex.exec(content))) {
          ...
          if (
            ...
            references.push({
              ...
              referenceType:
                'store-subscription' as CrossReference['referenceType'],
              fromLocation: {
                file: filePath,
                line: lineOffset + lineNum - 1,
                column:
      ...
      case 'vue':
        // Vue Composition API
        const vueCompositionRegex =
          /\b(ref|reactive|computed|watch|watchEffect|onMounted|onUnmounted|provide|inject)\s*\(/g
        ...
        const vueDefineRegex =
          /\b(defineProps|defineEmits|defineExpose|defineSlots)\s*(?:<[^>]*>)?\s*\(/g
      ...
      case 'astro':
        ...
        const astroGlobalRegex =
          /Astro\.(props|slots|request|params|cookies|redirect|response|glob)/g
  ...
  private extractTemplateSection(
    ...
    if (fileType === 'svelte') {
      ...
      const vueBindingRegex =
        /\{\{([^}]+)\}\}|v-model(?:\.(\w+))?="([^"]+)"|@(\w+)(?:\.(\w+))?|v-(if|for|show|else|else-if)="([^"]+)"/g
