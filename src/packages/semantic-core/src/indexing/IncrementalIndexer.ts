...
export class IncrementalIndexer {
  ...
  async updateIndex(diff: HashTreeDiff): Promise<void> {
    if (!this.silentMode) {
      console.error(
        `Updating index: +${diff.added.length} ~${diff.modified.length} -${diff.deleted.length}`
      )
    }
  ...
  async checkIntegrity(): Promise<{ consistent: boolean; issues: string[] }> {
    ...
    if (allHashes.size !== indexStats.totalFiles) {
      issues.push(
        `Hash tree has ${allHashes.size} files but semantic index has ${indexStats.totalFiles}`
      )
    }
