...
import { SemanticIndex, SemanticInfo, CrossReference } from './types'
...
export class FlowTracer {
   ...
   */
  private determineFlowType(info: SemanticInfo, searchTerm: string): FlowNode['type'] {
   ...
   */
  private extractVariableName(info: SemanticInfo, searchTerm: string): string | undefined {
   ...
   */
  private isDataFlowTarget(from: FlowNode, to: FlowNode, ref: CrossReference): boolean {
  ...
  formatFlowPath(path: FlowPath): string {
    ...
    path.nodes.forEach((node, index) => {
      ...
      output += `${stepNumber}. ${typeIcon} ${node.type.toUpperCase()}: ${node.location.file}:${node.location.line}\n`
      output += `   ${node.code}\n`
