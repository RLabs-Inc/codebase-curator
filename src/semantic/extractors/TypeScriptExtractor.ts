...
export class TypeScriptExtractor implements LanguageExtractor {
  ...
  private extractRelatedTerms(context: string): string[] {
    ...
    return [...new Set(matches)].filter(term => 
      term.length > 2 && 
      !['const', 'let', 'var', 'function', 'class', 'return', 'if', 'else', 'for', 'while'].includes(term)
    );
  }
  ...
  private isMeaningfulString(value: string): boolean {
    ...
    if (value.includes(' ') || value.includes('/') || value.includes('.') || value.length > 10) return true;
    return false;
  }
  ...
  private fallbackExtraction(content: string, filePath: string): { definitions: SemanticInfo[]; references: CrossReference[] } {
    ...
    lines.forEach((line, index) => {
      ...
      const functionMatches = [
        ...
        line.match(/(?:export\s+)?(?:const|let|var)\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=\s*(?:async\s+)?(?:\([^)]*\)|[a-zA-Z_$][a-zA-Z0-9_$]*)\s*=>/),
        line.match(/([a-zA-Z_$][a-zA-Z0-9_$]*)\s*:\s*(?:async\s+)?(?:function)?\s*\(/),
      ];
      ...
      // Extract strings
      const stringMatches = line.match(/"([^"]{4,})"|'([^']{4,})'|`([^`]{4,})`/g);
      if (stringMatches) {
