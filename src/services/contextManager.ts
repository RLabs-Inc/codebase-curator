...
export class ContextManager {
  ...
  constructor(options: CacheOptions = {}) {
    ...
    try {
      ...
      console.error(
        `[ContextManager] Failed to create cache directory at ${this.cacheDir}:`,
        error,
      );
      console.error(
        `[ContextManager] Caching will be disabled for this session`,
      );
...
Generate your instruction sentence now, and I'll format it for easy copying.`;
  ...
  private async detectFileChanges(
    ...
    if (metadata.hashTree) {
      const changedFiles = await this.detectChangesWithHashTree(
        projectPath,
        metadata.hashTree,
      );
    ...
    for (const [path, fileMeta] of currentFiles) {
      ...
      if (
        !oldMeta ||
        oldMeta.hash !== fileMeta.hash ||
        fileMeta.lastModified > since
      ) {
        return true;
      }
    }
      ...
      try {
        ...
        for (const [type, entry] of Object.entries(data)) {
          if (entry && typeof entry === "object" && "projectPath" in entry) {
            ...
            this.cache.get(projectPath)![type as keyof AnalysisCache] =
              entry as any;
          }
        }
      } catch (e) {
  ...
  getStats(): any {
    const stats = {
      ...
      totalCacheSize: 0,
      cachedAnalysisTypes: {} as Record<string, number>,
    };

    // Calculate stats from all cached projects
    for (const [projectPath, projectCache] of this.cache) {
      if (projectCache && projectCache.analysisCache) {
        for (const [key, entry] of projectCache.analysisCache) {
          const [type] = key.split("-");
          stats.cachedAnalysisTypes[type] =
            (stats.cachedAnalysisTypes[type] || 0) + 1;
        }
      }
    }

    // Note: Real implementation would track hits/misses during runtime
    return stats;
  }
   ...
   */
  private getOrCreateCache(projectPath: string): ProjectCache {
    if (!this.cache.has(projectPath)) {
      this.cache.set(projectPath, {
  ...
  async cacheAnalysis(
    ...
    const key = `${analysisType}-${projectPath}`;

    cache.analysisCache.set(key, {
      data,
      timestamp: Date.now(),
    });
  ...
  //  */
  // async getCachedAnalysis(projectPath: string, analysisType: string): Promise<any | null> {
  //   const cache = this.cache.get(projectPath);
  //   if (!cache || !cache.analysisCache) return null;

  //   const key = `${analysisType}-${projectPath}`;
  //   const entry = cache.analysisCache.get(key);
  ...
  //     cache.analysisCache.delete(key);
  //     return null;
  //   }
  ...
  clearCache(projectPath?: string): void {
    if (projectPath) {
      ...
      const cacheFile = join(
        this.cacheDir,
        `${this.hashPath(projectPath)}.json`,
      );
