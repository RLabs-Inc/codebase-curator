...
class IndexingMonitor {
  ...
  private async displayStatus(): Promise<void> {
    ...
    const timeSinceLastActivity = this.formatDuration(
    ...
    console.log('├─────────────────────────────────────────────────────────┤')
    console.log(
      `│ Uptime: ${uptime.padEnd(20)} │ Watching: ${
        status.isWatching ? '✅' : '❌'.padEnd(15)
      } │`
    )
    console.log(
      `│ Total Files: ${status.totalFiles
        .toString()
        .padEnd(13)} │ Indexed: ${status.indexedFiles.toString().padEnd(16)} │`
    )
    console.log(
      `│ Total Changes: ${this.stats.totalChanges
        .toString()
        .padEnd(11)} │ Last Activity: ${timeSinceLastActivity.padEnd(10)} │`
    )
    console.log(
      `│ Added: ${this.stats.filesAdded
        .toString()
        .padEnd(6)} Modified: ${this.stats.filesModified
        .toString()
        .padEnd(6)} Deleted: ${this.stats.filesDeleted.toString().padEnd(11)} │`
    )
    console.log('└─────────────────────────────────────────────────────────┘')
  }
  ...
  private async displayLiveOverview(): Promise<void> {
    ...
    const timeSinceOverviewUpdate = this.formatDuration(
      Date.now() - this.overviewData.lastUpdate
    )
    ...
    const uniqueFilesChanged =
      this.stats.uniqueFilesModified.size +
      this.stats.uniqueFilesAdded.size +
      this.stats.uniqueFilesDeleted.size
    console.log(
      `║ 📊 Tracked: ${status.indexedFiles
        .toString()
        .padEnd(3)} files │ Changed: ${uniqueFilesChanged
        .toString()
        .padEnd(2)} files │ Uptime: ${uptime.padEnd(8)} │ Watching: ${
        status.isWatching ? '✅' : '❌'
      } ║`
    )
    console.log(
      `║ 📈 Events: ${this.stats.totalChanges
        .toString()
        .padEnd(3)} │ Last Activity: ${timeSinceLastActivity.padEnd(
        6
      )} │ Overview: ${updateIndicator} ${timeSinceOverviewUpdate.padEnd(
        6
      )} ago     ║`
    )
    console.log(
    ...
    topTypes.forEach(([type, count]: [string, number]) => {
      const percentage = (
        (count / this.overviewData.totalDeclarations) *
        100
      ).toFixed(1)
      const bar = '█'.repeat(
        Math.floor(
          (count /
            Math.max(...this.overviewData.byType.map((t: any) => t[1]))) *
            25
        )
      )
      const line = `║   ${type.padEnd(12)} ${count
        .toString()
        .padStart(4)} (${percentage.padStart(5)}%) ${bar.padEnd(25)} ║`
      console.log(line)
    })
    ...
    topFiles.forEach(([file, count]: [string, number]) => {
      ...
      const truncatedPath =
        relativePath.length > 55
          ? '...' + relativePath.slice(-52)
          : relativePath
      console.log(
        `║   ${truncatedPath.padEnd(55)} ${count
          .toString()
          .padStart(3)} elements   ║`
      )
    })
    ...
    if (this.recentChanges.length > 0) {
      this.recentChanges.forEach((changeLog) => {
        const truncatedLog =
          changeLog.length > 77 ? changeLog.substring(0, 74) + '...' : changeLog
      ...
      console.log(
        `║   Events: ${this.stats.totalChanges
          .toString()
          .padEnd(2)} │ Unique files: ${uniqueFilesChanged
          .toString()
          .padEnd(2)} │ Saves: ${this.stats.filesModified
          .toString()
          .padEnd(2)} │ Additions: ${this.stats.filesAdded
          .toString()
          .padEnd(2)} │ Deletions: ${this.stats.filesDeleted
          .toString()
          .padEnd(2)} ║`
      )
      console.log(
  ...
  async stop(): Promise<void> {
    ...
    console.log('\n📊 Final Statistics:')
    console.log(
      `  Total runtime: ${this.formatDuration(
    ...
    console.log(
      `  Files saved: ${this.stats.filesModified} times (${this.stats.uniqueFilesModified.size} unique)`
    )
    console.log(
      `  Files added: ${this.stats.filesAdded} (${this.stats.uniqueFilesAdded.size} unique)`
    )
    console.log(
      `  Files deleted: ${this.stats.filesDeleted} (${this.stats.uniqueFilesDeleted.size} unique)`
    )
  }
  ...
  async showCodebaseOverview(): Promise<void> {
    ...
    sortedTypes.slice(0, 10).forEach(([type, count]) => {
      ...
      const bar = '█'.repeat(
        Math.floor((count / Math.max(...byType.values())) * 20)
      )
      console.log(
    ...
    sortedFiles.slice(0, 10).forEach(([file, count]) => {
      ...
      const relativePath = this.relativePath(file)
      console.log(
        `  ${relativePath.padEnd(40)} ${count
          .toString()
          .padStart(3)} declarations, ${typeCount} types`
      )
    })
    ...
    for (const term of searchTerms) {
      const results = semanticService.search(term, {
        type: [term],
        maxResults: 5,
      })
      if (results.length > 0) {
        ...
        results.forEach((result) => {
          if (result.usageCount && result.usageCount > 0) {
            console.log(
              `    ${result.info.term} (${
                result.usageCount
              } refs) - ${this.relativePath(result.info.location.file)}:${
                result.info.location.line
              }`
            )
          }
        })
  ...
  async showDetailedStatus(): Promise<void> {
    ...
    console.log('\n📊 Activity Statistics:')
    console.log(
      `  Runtime: ${this.formatDuration(Date.now() - this.stats.startTime)}`
    )
    ...
    console.log('\n🔍 Index Integrity:')
    console.log(
      `  Status: ${integrity.consistent ? '✅ Consistent' : '❌ Issues found'}`
    )
    ...
    try {
      ...
      const hashTreeSize = (await hashTreeFile.exists())
        ? `${Math.round((await hashTreeFile.size()) / 1024)}KB`
        : 'Not found'
    ...
    try {
      ...
      const indexSize = (await indexFile.exists())
        ? `${Math.round((await indexFile.size()) / 1024)}KB`
        : 'Not found'
...
async function main() {
  ...
  switch (command) {
    ...
    case 'monitor':
      console.log(
        `🚀 Starting incremental indexing monitor for: ${projectPath}\n`
      )
    ...
    default:
      ...
      console.log('Usage:')
      console.log(
        '  bun run src/semantic/monitor.ts watch [project-path]           # Basic real-time monitoring'
      )
      console.log(
        '  bun run src/semantic/monitor.ts watch --overview [project-path] # Live monitoring + overview'
      )
      console.log(
        '  bun run src/semantic/monitor.ts overview [project-path]        # Static codebase overview'
      )
      console.log(
        '  bun run src/semantic/monitor.ts status [project-path]          # Detailed technical status'
      )
      console.log(
        '  bun run src/semantic/monitor.ts rebuild [project-path]         # Force rebuild index'
      )
      console.log('')
      console.log('Examples:')
      console.log(
        '  bun run src/semantic/monitor.ts watch                    # Basic monitoring'
      )
      console.log(
        '  bun run src/semantic/monitor.ts watch --overview         # Live overview dashboard'
      )
      console.log(
        '  bun run src/semantic/monitor.ts watch -o /path/to/project # Live overview for specific path'
      )
      console.log(
        '  bun run src/semantic/monitor.ts overview                 # Static analysis snapshot'
      )
      console.log('')
      console.log(
        'The monitor will show real-time file changes and reindexing activity.'
      )
