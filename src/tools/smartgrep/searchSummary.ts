...
export class SearchSummaryGenerator {
  ...
  private describeFileRole(file: string, types: string[], results: SearchResult[]): string {
    ...
    if (types.includes('class') && results.some(r => r.relevanceScore > 0.9)) {
  ...
  private findEcosystem(results: SearchResult[]): EcosystemInfo {
    ...
    const functionCalls = results.filter(r => 
      r.info.context.includes('(') && r.info.context.includes(')')
    )
    
    if (functionCalls.length > results.length / 2) {
      // Extract common call pattern
      const callPatterns = functionCalls.map(r => {
      ...
      if (mostCommon) {
        patterns.push(`${mostCommon}() → returns ${this.guessReturnType(results)}`)
      }
    }
  ...
  private generateSuggestions(
    ...
    const commonTerms = ['export', 'import', 'from', 'const', 'let', 'var', 'this', 'await', 'return']
    const meaningfulRelated = analysis.ecosystem.relatedTerms.filter(term => 
  ...
  private formatSummary(summary: SearchSummary, context: SearchContext): string {
    const { query } = context
    let output = '\n'
    output += '═'.repeat(70) + '\n'
    ...
    if (summary.topLocations.length > 0) {
      output += `🔥 THE ${Math.min(3, summary.topLocations.length)} PLACES THAT MATTER MOST:\n`
      summary.topLocations.forEach((loc, i) => {
        const count = loc.occurrences > 1 ? ` (${loc.occurrences} times)` : ''
        output += `   ${i + 1}. ${loc.file}:${loc.line} → ${loc.description.toUpperCase()}${count}\n`
      })
      output += '\n'
    }
    ...
    if (summary.impactAnalysis.breakageRisk.length > 0) {
      ...
      if (summary.impactAnalysis.totalOccurrences > summary.impactAnalysis.breakageRisk.length) {
        const more = summary.impactAnalysis.totalOccurrences - summary.impactAnalysis.breakageRisk.length
    ...
    // Ecosystem
    if (summary.ecosystem.relatedTerms.length > 0 || summary.ecosystem.commonPatterns.length > 0) {
      ...
      if (summary.ecosystem.throwsErrors && summary.ecosystem.throwsErrors.length > 0) {
