...
export class StoryDisplay {
      ...
      flow.stages.forEach((stage, i) => {
        ...
        output.push(
          chalk.blue('│   ') +
            narrativeType +
            ' ' +
            chalk.yellow(this.truncateString(stage.pattern, 50))
        )
        ...
        if (!isLast) {
          if (stage.errorStages.length > 0) {
            output.push(
              chalk.blue('│   ') +
                '  ├─❌ ' +
                chalk.red(this.truncateString(stage.errorStages[0], 45))
            )
          }
          output.push(chalk.blue('│   ') + '  ↓')
        }
  ...
  private displayActors(actors: StoryActor[]): string {
    ...
    Object.entries(grouped).forEach(([type, typeActors]) => {
      ...
      output.push(
        chalk.green('│ ') + chalk.bold(this.getActorTypeDisplay(type))
      )
      ...
      typeActors.forEach((actor) => {
        ...
        samples.forEach((interaction) => {
          output.push(
            chalk.green('│     ') +
              chalk.gray(this.truncateString(interaction, 40))
          )
        })
      })
    })
  ...
  private displayConflicts(conflicts: ConflictPattern[]): string {
    ...
    conflicts.slice(0, 5).forEach((conflict) => {
      ...
      output.push(
        chalk.red('│ ') +
          chalk.bold(
            this.getConflictIcon(conflict.category) +
              ' ' +
              this.titleCase(conflict.category)
          ) +
          chalk.gray(` (${conflict.frequency} occurrences)`)
      )
      ...
      conflict.triggers.slice(0, 3).forEach((trigger) => {
        output.push(
          chalk.red('│   ') +
            '• ' +
            chalk.yellow(this.truncateString(trigger, 45))
        )
      })
    })
  ...
  private displayStates(states: any[]): string {
    ...
    output.push(
      chalk.magenta('┌─ 🔄 STATE TRANSITIONS ──────────────────────────┐')
    )
    ...
    states.slice(0, 5).forEach((transition) => {
      output.push(
        chalk.magenta('│   ') +
          chalk.yellow(transition.fromState) +
          ' → ' +
          chalk.green(transition.toState) +
    ...
    patterns.forEach((pattern) => {
      if (pattern.patterns.length > 0) {
        ...
        output.push(
          chalk.cyan('│ ') +
            chalk.bold(
              this.getNarrativeIcon(pattern.type) +
                ' ' +
                this.titleCase(pattern.type)
            ) +
            chalk.gray(` (${pattern.patterns.length} instances)`)
        )
        ...
        pattern.patterns.slice(0, 3).forEach((example) => {
          output.push(
            chalk.cyan('│   ') +
              '• ' +
              chalk.white(this.truncateString(example, 45))
          )
        })
      }
    })
